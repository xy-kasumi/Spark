...
async function initWasmModule(): Promise<any> {
    if (!wasmModule) {
        ...
        wasmModule = await WasmGeomModule({
            env: {
 ...
 */
export function fromTriSoup(triSoup: Float64Array): THREE.BufferGeometry {
...
/**
 * Internal WASM wrapper for mesh projection
 */
async function callWasmProjectMeshInternal(
...
/**
 * Internal WASM wrapper for Manifold subtraction
 */
async function callWasmManifoldSubtractInternal(
    meshA: Float64Array,
    meshB: Float64Array
 ...
 */
export async function callWasmProjectMesh(
    ...
    return callWasmProjectMeshInternal(triSoup, origin, viewX, viewY, viewZ);
}
 ...
 */
export async function callWasmManifoldSubtract(
    geometryA: THREE.BufferGeometry,
    geometryB: THREE.BufferGeometry
): Promise<THREE.BufferGeometry> {
    const soupA = toTriSoup(geometryA);
    const soupB = toTriSoup(geometryB);
    const resultSoup = await callWasmManifoldSubtractInternal(soupA, soupB);
    return fromTriSoup(resultSoup);
}